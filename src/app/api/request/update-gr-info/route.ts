...
export async function POST(req: Request) {
    try {
        ...
        const senderName = getString(formData.get("senderName"));
        ...
        await prisma.repair_request.update({
            where: { id: idNum },
            data: {
                cs_send_to_gr_by: senderName,
                cs_to_gr_receive_by: receiverName,
                cs_to_gr_tel: receiverTel,
        ...
        const trans_log_text = "เพิ่มข้อมูล GR รับสินค้าซ่อมจาก CS ใบแจ้งซ่อม : " + res.request_no; 
    ...
    console.error("update gr info error:", err);
