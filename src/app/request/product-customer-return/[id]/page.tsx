...
import { ShieldCheck, ShieldX, ArrowRight, Eye, FileText, Trash2 } from "lucide-react";
...
export default function RequestVendorDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        fetch("/api/maintain/example-images?flow=product_customer_return")
            .then(res => res.json())
            .then(data => {
            ...
            try {
                ...
                if (raw) {
                    ...
                    if (alive) {
                        ...
                        setSentName(u.user_full_name || u.user_name || "");
                    }
                }
    ...
    const onSave = async () => {
        ...
        if (!customerName.trim())   nextErrors.customerName   = "กรุณากรอกชื่อผู้รับสินค้าคืน";
        if (!sentName.trim())    nextErrors.sentName    = "กรุณากรอกชื่อผู้ส่งมอบสินค้าคืน";
        ...
        try {
            ...
            const mode = statusNum === 290 ? "DC" : "VEN";
            ...
            selectedFiles.forEach((file) => {
                formData.append("attachments", file); // Expects "attachments"
            });
    ...
    return (
        <section className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
                ...
                <span>/</span>
                <span>ส่งคืนสินค้า</span>
                <span>/</span>
                <span className="text-[#c8102e]">ส่งคืนเครื่องให้ลูกค้า</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    บันทึกส่งมอบเครื่องซ่อมคืนลูกค้า (Customer Return)
                </h1>
                <p className="text-xs text-slate-400 font-medium pl-3.5">
                    ขั้นตอนสุดท้าย: ตรวจสอบการรับเงินค้างชำระ และลงชื่อส่งสินค้าคืนลูกค้าโดยฝ่ายบริการลูกค้า (CS)
                </p>
            </div>

            {/* Outstanding Balance check panel */}
            <div className="bg-white border border-slate-200/80 border-t-4 border-t-[#c8102e] rounded-2xl p-6 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                        💳 ตรวจสอบสถานะการเงิน (Outstanding Balance check)
                    </h3>
                    {outstanding > 0.01 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-150 text-center">
                        <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider">ยอดค่าซ่อมสุทธิ</span>
                        <span className="text-base font-black text-slate-800 tabular-nums">{quotationTotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })} บาท</span>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-150 text-center">
                        <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider">ยอดชำระแล้ว</span>
                        <span className="text-base font-black text-emerald-700 tabular-nums">{paidTotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })} บาท</span>
                    </div>
                    <div className={`rounded-xl p-3 border text-center ${outstanding > 0.01 ? 'bg-red-50 border-red-150' : 'bg-slate-50 border-slate-150'}`}>
                        <span className={`text-[10px] font-bold block uppercase tracking-wider ${outstanding > 0.01 ? 'text-red-500' : 'text-slate-400'}`}>ยอดคงเหลือค้างชำระ</span>
                        <span className={`text-base font-black tabular-nums ${outstanding > 0.01 ? 'text-red-600' : 'text-slate-800'}`}>{outstanding.toLocaleString("th-TH", { minimumFractionDigits: 2 })} บาท</span>
                    </div>
                </div>

                {paymentsList.length > 0 && (
                    <div className="space-y-1.5">
                        <span className="text-[10px] font-extrabold text-slate-450 block uppercase tracking-wider">ประวัติการชำระเงิน :</span>
                                ...
                                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                                    {paymentsList.map((p, idx) => (
                                        <tr key={p.payment_id || idx}>
                                            ...
                                            <td className="px-4 py-2 text-slate-500">{p.payment_type}</td>
                                            <td className="px-4 py-2 text-slate-500">{p.method}</td>
                                            <td className="px-4 py-2 text-slate-500">{p.ref_no || "-"}</td>
                                            <td className="px-4 py-2 text-right text-slate-900 tabular-nums">{(p.amount || 0).toLocaleString("th-TH", { minimumFractionDigits: 2 })} บาท</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                {outstanding > 0.01 && (
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 mt-2 space-y-3">
                        <span className="text-xs font-black text-[#c8102e] block">💳 บันทึกชำระยอดค้างจ่ายส่วนที่เหลือ</span>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-slate-500 mb-1">เลขอ้างอิงสลิป / รูดบัตร (ถ้ามี)</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <button
                                    ...
                                    className="w-full h-[38px] rounded-xl bg-slate-900 text-white hover:bg-black font-bold text-xs shadow-sm transition flex items-center justify-center cursor-pointer"
                                >
            ...
            <div className="bg-white border border-slate-200/80 border-t-4 border-t-[#c8102e] rounded-2xl p-6 md:p-8 shadow-sm">
                <form className="space-y-8">
                    ...
                    {/* Section 3: Customer return hand-over details */}
                    <div>
                        <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100 mb-6">
                            <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                            <h3 className="text-sm font-bold text-slate-800">
                                🚚 บันทึกการส่งมอบสินค้าและเซ็นชื่อรับเครื่องคืน
                            </h3>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label htmlFor="sentName" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้ส่งมอบสินค้าคืน (เจ้าหน้าที่ CS)<Req /></label>
                                <input
                                    id="sentName"
                                ...
                                />
                                {errors.sentName && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.sentName}</p>}
                            </div>
                            <div>
                                <label htmlFor="customerName" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้รับสินค้าคืน (ลูกค้า/ตัวแทนรับของ)<Req /></label>
                                <input
                                    id="customerName"
                                ...
                                />
                                {errors.customerName && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.customerName}</p>}
                            </div>
                            <div>
                                <label htmlFor="sentProductReturnDate" className="block text-xs font-bold text-slate-700 mb-1.5">วันที่ลูกค้ารับของคืน<Req /></label>
                                <DatePicker
                                ...
                                />
                                {errors.sentProductReturnDate && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.sentProductReturnDate}</p>}
                            </div>
                        </div>

                        <div className="mt-6 space-y-2 max-w-lg">
                            <label className="block text-xs font-bold text-slate-700 mb-1">✍ ลายเซ็นดิจิทัลผู้รับสินค้าคืน (Signature pad)</label>
                            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-2.5 flex flex-col space-y-2">
                                <div className="w-full h-44 rounded-xl overflow-hidden border border-slate-200 bg-white">
                                    <SignatureCanvas
                                        ref={sigRef}
                                        penColor="black"
                                        canvasProps={{
                                ...
                                <div className="flex justify-end">
                                    <button
                                        ...
                                        className="px-3.5 py-1.5 rounded-lg border border-slate-200 text-slate-500 hover:text-slate-700 bg-white text-xs font-bold transition shadow-sm cursor-pointer"
                                    >
                                        ล้างลายเซ็น
                                    </button>
                                </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between pb-2.5 border-b border-slate-100 mb-6">
                            <div className="flex items-center gap-2">
                                <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                                <h3 className="text-sm font-bold text-slate-800">
                                    📷 ถ่ายภาพตรวจรับสินค้าของลูกค้าเป็นหลักฐาน (Attachments)*
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {renderUploadSlot('slot4', 'เอกสารมอบของ/ใบรับประกันใหม่*', true, fileSlot4, previewSlot4, setFileSlot4, slot4Ref)}
                        </div>
                        {errors.attachments && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider hidden md:inline-block">ขั้นตอนปัจจุบัน: Customer Return</span>
                    <button
                        type="button"
                        onClick={onSave}
                    ...
                    >
                        บันทึกและปิดใบงานสำเร็จ <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                </div>
            </div>
