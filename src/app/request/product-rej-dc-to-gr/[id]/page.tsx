...
export default function DcRejReturnGrPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const onSave = async () => {
        ...
        if (!dcSentTel.trim())    nextErrors.dcSentTel    = "กรุณากรอกเบอร์ติดต่อผู้ส่ง";
        ...
        try {
            ...
            formData.append("action", "ProductRejDcToHO");
            ...
            const res = await fetch("/api/request/update-product-return-rej", {
                method: "POST",
                body: formData, 
            });
    ...
    return (
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-center text-slate-800">
            เพิ่มรายละเอียดการส่งคืนสินค้า (GR)
        </h1>
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-6 md:p-8 shadow-sm">
            ...
            <fieldset className="space-y-4">
                <legend className="text-lg font-semibold text-slate-900">
                    รายละเอียดบันทึกการส่งคืนสินค้าจาก DC (Reject)
                </legend>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        />
                        {errors.dcSentName && <p className="text-red-600 text-sm mt-1">{errors.dcSentName}</p>}
                    </div>
                    <div>
                    ...
                    <div>
                        <label htmlFor="dcReturnDate" className="form-label block mb-1">วันที่รับสินค้าคืน<Req /></label>
                            ...
                            />
                            {errors.dcReturnDate && <p className="text-red-600 text-sm mt-1">{errors.dcReturnDate}</p>}
                    </div>
                    <div>
                        <label htmlFor="dcSentTel" className="form-label">เบอร์ติดต่อผู้ส่งมอบ<Req /></label>
                        <input
                            id="dcSentTel"
                            className={`${inputClass(!!errors.dcSentTel)} w-50`}
                            value={dcSentTel}
                            onChange={e => setDcSentTel(e.target.value)}
                        />
                        {errors.dcSentTel && <p className="text-red-600 text-sm mt-1">{errors.dcSentTel}</p>}
                    </div>
                </div>
                <div className="pt-2">
