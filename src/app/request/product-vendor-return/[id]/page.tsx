...
export default function RequestVendorDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        fetch("/api/maintain/example-images?flow=product_vendor_return")
            .then(res => res.json())
            .then(data => {
            ...
            try {
                ...
                if (raw) {
                    ...
                    if (alive) {
                        ...
                        setReceiverName(u.user_full_name || u.user_name || "");
                    }
                }
    ...
    const onSave = async () => {
        ...
        try {
            ...
            const mode = [234, 235].includes(statusNum) ? "DC" : "VEN";
            const vendorReturnDateStr = vendorReturnDate ? toYMD(vendorReturnDate) : "";

            const formData = new FormData();
            formData.append("action", "ProductVendorReturn");
    ...
    return (
        <section className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
                ...
                <span>/</span>
                <span className="text-[#c8102e]">รับคืนจาก Vendor</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    บันทึกรับคืนสินค้าจากผู้ผลิต (Vendor Returns)
                </h1>
                <p className="text-xs text-slate-400 font-medium pl-3.5">
                    บันทึกขั้นตอนการรับเครื่องซ่อมเสร็จหรือส่งคืนจากทาง Vendor กลับเข้าสู่ระบบคลัง DC/GR
                </p>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 border-t-4 border-t-[#c8102e] rounded-2xl p-6 md:p-8 shadow-sm">
                <form className="space-y-8">
                    ...
                    {/* Section 3: Vendor Return info */}
                    <div>
                        <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100 mb-6">
                            <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                            <h3 className="text-sm font-bold text-slate-800">
                                🚚 บันทึกข้อมูลการรับคืนสินค้าจาก Vendor
                            </h3>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="vendorSentName" className="block text-xs font-bold text-slate-700 mb-1.5">ชื่อผู้ส่งมอบ (ตัวแทน Vendor)<Req /></label>
                                <input
                                    id="vendorSentName"
                                ...
                                />
                                {errors.vendorSentName && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.vendorSentName}</p>}
                            </div>
                            <div>
                                <label htmlFor="receiverName" className="block text-xs font-bold text-slate-700 mb-1.5">ชื่อผู้รับสินค้าคืน<Req /></label>
                                <input
                                    id="receiverName"
                            ...
                            <div>
                                <label htmlFor="vendorReturnDate" className="block text-xs font-bold text-slate-700 mb-1.5">วันที่รับคืนสินค้าซ่อมเสร็จ<Req /></label>
                                <DatePicker
                                    id="vendorReturnDate"
                                ...
                                />
                                {errors.vendorReturnDate && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.vendorReturnDate}</p>}
                            </div>
                            <div>
                                <label htmlFor="vendorSentTel" className="block text-xs font-bold text-slate-700 mb-1.5">เบอร์ติดต่อผู้ส่งคืน</label>
                                <input
                                    id="vendorSentTel"
                                    className={inputClass(!!errors.vendorSentTel)}
                                    value={vendorSentTel}
                                    onChange={e => setVendorSentTel(e.target.value)}
                                />
                                {errors.vendorSentTel && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.vendorSentTel}</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between pb-2.5 border-b border-slate-100 mb-6">
                            <div className="flex items-center gap-2">
                                <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                                <h3 className="text-sm font-bold text-slate-800">
                                    📷 ถ่ายภาพตัวเครื่องและใบเสร็จซ่อมจาก Vendor (Attachments)*
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {renderUploadSlot('slot4', 'ใบรับรองการซ่อมเสร็จ/คืนของจาก Vendor*', true, fileSlot4, previewSlot4, setFileSlot4, slot4Ref)}
                        </div>
                        {errors.attachments && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider hidden md:inline-block">ขั้นตอนปัจจุบัน: Vendor Return</span>
                    <button
                        type="button"
                        onClick={onSave}
                    ...
                    >
                        บันทึกรับของคืนสำเร็จ <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                </div>
            </div>
