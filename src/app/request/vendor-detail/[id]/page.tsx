...
import { ShieldCheck, ShieldX, ArrowRight, Eye } from "lucide-react";
...
export default function VendorDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        fetch("/api/maintain/example-images?flow=vendor_detail")
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <section className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
                ...
                <span>Vendor</span>
                <span>/</span>
                <span className="text-[#c8102e]">รับเครื่องซ่อม</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    บันทึก Vendor รับมอบงานซ่อม (Vendor Pickup)
                </h1>
                <p className="text-xs text-slate-400 font-medium pl-3.5">
                    ลงบันทึกการตรวจรับมอบสินค้าของแผนกช่างซ่อม Vendor ผู้ผลิตเข้าคลังซ่อมอย่างเป็นทางการ
                </p>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 border-t-4 border-t-[#c8102e] rounded-2xl p-6 md:p-8 shadow-sm">
                <form className="space-y-8">
                    ...
                    {/* Section 3: Vendor pickup details */}
                    <div>
                        <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100 mb-6">
                            <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                            <h3 className="text-sm font-bold text-slate-800">
                                🚚 ข้อมูลการตรวจรับสินค้าโดยช่าง Vendor
                            </h3>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="vendorName" className="block text-xs font-bold text-slate-700 mb-1.5">ชื่อร้านค้า Vendor/ผู้ผลิต<Req /></label>
                                <input
                                    id="vendorName"
                            ...
                            <div>
                                <label htmlFor="receiverName" className="block text-xs font-bold text-slate-700 mb-1.5">ชื่อผู้บันทึกตรวจรับสินค้า<Req /></label>
                                <input
                                    id="receiverName"
                            ...
                            <div>
                                <label htmlFor="sendDate" className="block text-xs font-bold text-slate-700 mb-1.5">วันที่ตรวจรับสินค้ามอบซ่อม<Req /></label>
                                <DatePicker
                                    id="sendDate"
                                    selected={sendDate}
                            ...
                            <div>
                                <label htmlFor="receiverTel" className="block text-xs font-bold text-slate-700 mb-1.5">เบอร์ติดต่อหน่วยซ่อม Vendor<Req /></label>
                                <input
                                    id="receiverTel"
                                    className={inputClass(!!errors.receiverTel)}
                                    value={receiverTel}
                                    onChange={e => setReceiverTel(e.target.value)}
                                />
                                {errors.receiverTel && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.receiverTel}</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between pb-2.5 border-b border-slate-100 mb-6">
                            <div className="flex items-center gap-2">
                                <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                                <h3 className="text-sm font-bold text-slate-800">
                                    📷 ถ่ายภาพตัวสินค้าประกอบการรับมอบซ่อมโดย Vendor (Attachments)*
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {renderUploadSlot('slot4', 'ใบรับงาน/เอกสารตรวจรับโดย Vendor*', true, fileSlot4, previewSlot4, setFileSlot4, slot4Ref)}
                        </div>
                        {errors.attachments && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider hidden md:inline-block">ขั้นตอนปัจจุบัน: Vendor Pickup</span>
                    <button
                        type="button"
                        onClick={onSave}
                    ...
                    >
                        บันทึกรับมอบเครื่อง <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                </div>
            </div>
