...
import { ShieldCheck, ShieldX, ArrowRight, Eye, Undo2, Check } from "lucide-react";
...
export default function GrToDcLogPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        fetch("/api/maintain/example-images?flow=gr_log_to_dc")
            .then(res => res.json())
            .then(data => {
            ...
            try {
                ...
                if (raw) {
                    ...
                    if (alive) {
                        ...
                        setOpenLogBy(u.user_full_name || u.user_name || "");
                    }
                    setUserInfo({
    ...
    return (
        <section className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
                ...
                <span>คลังสาขา</span>
                <span>/</span>
                <span className="text-[#c8102e]">เปิด Log DC</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    บันทึกรายละเอียดการเปิด Log ให้ DC (GR to DC Log)
                </h1>
                <p className="text-xs text-slate-400 font-medium pl-3.5">
                    เปิด Log ข้อมูลสินค้าซ่อมเพื่อจัดส่งสินค้าเข้าคลัง DC กลางในขั้นตอนต่อไป
                </p>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 border-t-4 border-t-[#c8102e] rounded-2xl p-6 md:p-8 shadow-sm">
                <form className="space-y-8">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label htmlFor="openLogBy" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้เปิด Log<Req /></label>
                                <input
                                    id="openLogBy"
                                ...
                                />
                                {errors.openLogBy && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.openLogBy}</p>}
                            </div>
                            <div>
                                <label htmlFor="openLogDate" className="block text-xs font-bold text-slate-700 mb-1.5">วันที่เปิด Log<Req /></label>
                                <DatePicker
                                    id="openLogDate"
                                ...
                                />
                                {errors.openLogDate && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.openLogDate}</p>}
                            </div>
                            <div>
                                <label htmlFor="approvelogID" className="block text-xs font-bold text-slate-700 mb-1.5">เลขที่ขออนุมัติส่งซ่อม (Log ID)<Req /></label>
                                <input
                                    id="approvelogID"
                                    className={inputClass(!!errors.approvelogID)}
                                    value={approvelogID}
                                    onChange={e => setApprovelogID(e.target.value)}
                                />
                                {errors.approvelogID && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.approvelogID}</p>} 
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between pb-2.5 border-b border-slate-100 mb-6">
                            <div className="flex items-center gap-2">
                                <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                                <h3 className="text-sm font-bold text-slate-800">
                                    📷 ถ่ายภาพประกบกล่องและบันทึก Serial Tag (Attachments)*
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {renderUploadSlot('slot4', 'เอกสารส่งมอบ/ใบเสร็จ Serial*', true, fileSlot4, previewSlot4, setFileSlot4, slot4Ref)}
                        </div>
                        {errors.attachments && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
                <div className="flex gap-2">
                    ...
                    {((userInfo?.role === "ADMIN" || userInfo?.role === "ADMIN GR" || userInfo?.role === "GR") 
                        && (statusNum === 20 || statusNum === 200 || statusNum === 110) ) && (
                        <button
                            type="button"
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider hidden md:inline-block">ขั้นตอนปัจจุบัน: GR to DC Log</span>
                    <button
                        type="button"
                        onClick={onSave}
                    ...
                    >
                        บันทึกและส่งต่อไปยัง DC <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 border border-slate-200">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea
                                ...
                                placeholder="เช่น เอกสารแนบไม่สมบูรณ์ หรือ ข้อมูลสินค้าไม่ถูกต้อง..."
                            />
                        </div>
