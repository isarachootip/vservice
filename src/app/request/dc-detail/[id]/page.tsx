...
export default function DcDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        fetch("/api/maintain/example-images?flow=dc_detail")
            .then(res => res.json())
            .then(data => {
            ...
            try {
                ...
                if (raw) {
                    ...
                    if (alive) {
                        ...
                        setSentToDCName(u.user_full_name || u.user_name || "");
                    }
                    setUserInfo({
    ...
    return (
        <section className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
                ...
                <span>คลัง DC</span>
                <span>/</span>
                <span className="text-[#c8102e]">รับมอบสินค้าจากสาขา</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    บันทึกการส่งมอบสินค้าเข้าคลัง DC (DC Pickup)
                </h1>
                <p className="text-xs text-slate-400 font-medium pl-3.5">
                    ลงบันทึกข้อมูลการรับมอบสินค้าซ่อมจากแผนก GR ของสาขาเข้าสู่ระบบโลจิสติกส์ของ DC
                </p>
            </div>
                ...
                <form className="space-y-8">
                    ...
                    <div>
                        <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100 mb-6">
                            <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                            <h3 className="text-sm font-bold text-slate-800">
                                🚚 รายละเอียดการรับของจากสาขาเข้าระบบ DC
                            </h3>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label htmlFor="sentToDCName" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้มารับสินค้า (เจ้าหน้าที่ DC)<Req /></label>
                                <input
                                    id="sentToDCName"
                                ...
                                />
                                {errors.sentToDCName && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.sentToDCName}</p>}
                            </div>
                            <div>
                                <label htmlFor="dcReceiveDate" className="block text-xs font-bold text-slate-700 mb-1.5">วันที่รับสินค้าจากสาขา<Req /></label>
                                <DatePicker
                                    id="dcReceiveDate"
                                ...
                                />
                                {errors.dcReceiveDate && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.dcReceiveDate}</p>}
                            </div>
                            <div>
                                <label htmlFor="receiverTel" className="block text-xs font-bold text-slate-700 mb-1.5">เบอร์ติดต่อผู้มารับสินค้า<Req /></label>
                                <input
                                    id="receiverTel"
                    ...
                    <div>
                        <div className="flex items-center justify-between pb-2.5 border-b border-slate-100 mb-6">
                            <div className="flex items-center gap-2">
                                <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                                <h3 className="text-sm font-bold text-slate-800">
                                    📷 ถ่ายภาพประกอบการตรวจรับของ DC (Attachments)*
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {renderUploadSlot('slot4', 'เอกสารรับรองการจัดส่งของ DC*', true, fileSlot4, previewSlot4, setFileSlot4, slot4Ref)}
                        </div>
                        {errors.attachments && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider hidden md:inline-block">ขั้นตอนปัจจุบัน: DC Pickup</span>
                    <button
                        type="button"
                        onClick={onSave}
                    ...
                    >
                        บันทึกสินค้าออกเดินทาง <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                </div>
            </div>
