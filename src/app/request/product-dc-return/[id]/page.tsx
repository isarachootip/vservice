...
import { ShieldCheck, ShieldX, ArrowRight, Eye, FileText } from "lucide-react";
...
export default function DcReturnDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    useEffect(() => {
        fetch("/api/maintain/example-images?flow=product_dc_return")
            .then(res => res.json())
            .then(data => {
            ...
            try {
                ...
                if (raw) {
                    ...
                    if (alive) {
                        ...
                        setDcSentName(u.user_full_name || u.user_name || "");
                    }
                }
    ...
    const onSave = async () => {
        ...
        if (!dcSentName.trim())   nextErrors.dcSentName   = "กรุณากรอกชื่อผู้ส่งมอบ";
        if (!receiverName.trim())    nextErrors.receiverName    = "กรุณากรอกชื่อผู้รับ";
        if (!dcSentTel.trim())    nextErrors.dcSentTel    = "กรุณากรอกเบอร์ผู้ส่งมอบ";
        if (!dcReturnDate)  nextErrors.dcReturnDate    = "กรุณาเลือกวันที่รับสินค้าคืน";
        ...
        try {
            ...
            let mode = "";
            if(statusNum === 280 || statusNum === 2360){
                mode = "DC";
            } else {
                mode = "VEN";
            }
            const dcReturnDateStr = dcReturnDate ? toYMD(dcReturnDate) : "";

            const formData = new FormData();
            formData.append("action", "ProductDcReturn");
            formData.append("requestId", String(id));
            formData.append("receiverName", receiverName);
            formData.append("dcSentName", dcSentName);
            formData.append("dcReturnDate", dcReturnDateStr);
            formData.append("dcSentTel", dcSentTel);
    ...
    return (
        <section className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
                ...
                <span>/</span>
                <span className="text-[#c8102e]">DC ส่งมอบคืนสาขา</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    บันทึก DC ส่งคืนสินค้ากลับสาขา (DC Return)
                </h1>
                <p className="text-xs text-slate-400 font-medium pl-3.5">
                    ลงบันทึกการตีกลับหรือจัดส่งสินค้าซ่อมเสร็จจากคลัง DC ไปยังแผนกคลังสินค้าของสาขา (GR)
                </p>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 border-t-4 border-t-[#c8102e] rounded-2xl p-6 md:p-8 shadow-sm">
                <form className="space-y-8">
                    ...
                    {/* Section 3: DC Return details */}
                    <div>
                        <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100 mb-6">
                            <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                            <h3 className="text-sm font-bold text-slate-800">
                                🚚 บันทึกรายละเอียดการจัดส่งสินค้าคืนให้แก่สาขา
                            </h3>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="dcSentName" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้ส่งมอบ (เจ้าหน้าที่ DC)<Req /></label>
                                <input
                                    id="dcSentName"
                                ...
                                />
                                {errors.dcSentName && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.dcSentName}</p>}
                            </div>
                            <div>
                                <label htmlFor="receiverName" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้รับสินค้า (เจ้าหน้าที่ GR สาขา)<Req /></label>
                                <input
                                    id="receiverName"
                            ...
                            <div>
                                <label htmlFor="dcReturnDate" className="block text-xs font-bold text-slate-700 mb-1.5">วันที่รับของคืนถึงสาขา<Req /></label>
                                <DatePicker
                                    id="dcReturnDate"
                                ...
                                />
                                {errors.dcReturnDate && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.dcReturnDate}</p>}
                            </div>
                            <div>
                                <label htmlFor="dcSentTel" className="block text-xs font-bold text-slate-700 mb-1.5">เบอร์ติดต่อผู้ส่งมอบ (DC)<Req /></label>
                                <input
                                    id="dcSentTel"
                                ...
                                />
                                {errors.dcSentTel && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.dcSentTel}</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between pb-2.5 border-b border-slate-100 mb-6">
                            <div className="flex items-center gap-2">
                                <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                                <h3 className="text-sm font-bold text-slate-800">
                                    📷 ถ่ายภาพกล่องพัสดุและบาร์โค้ดสินค้าขณะรับคืน (Attachments)*
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {renderUploadSlot('slot4', 'ใบนำส่งคืนคลัง GR สาขา*', true, fileSlot4, previewSlot4, setFileSlot4, slot4Ref)}
                        </div>
                        {errors.attachments && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider hidden md:inline-block">ขั้นตอนปัจจุบัน: DC Return</span>
                    <button
                        type="button"
                        onClick={onSave}
                    ...
                    >
                        บันทึกของคืนถึงสาขาสำเร็จ <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                </div>
            </div>
