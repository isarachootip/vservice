...
export default function GrReturnCsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        fetch("/api/maintain/example-images?flow=product_gr_to_cs_return")
            .then(res => res.json())
            .then(data => {
            ...
            try {
                ...
                if (raw) {
                    ...
                    if (alive) {
                        ...
                        setGrSentName(u.user_full_name || u.user_name || "");
                    }
                }
    ...
    const onSave = async () => {
        ...
        try {
            ...
            let mode = "";
            if(statusNum === 285 || statusNum === 2361){
                mode = "DC";
            } else {
                mode = "VEN";
            }
    ...
    return (
        <section className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
                ...
                <span>/</span>
                <span className="text-[#c8102e]">GR ส่งมอบคืน CS</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    บันทึก GR ส่งคืนสินค้าคืนฝ่าย CS (GR Return to CS)
                </h1>
                <p className="text-xs text-slate-400 font-medium pl-3.5">
                    ลงบันทึกการส่งมอบสินค้ากลับคืนให้แก่ฝ่ายบริการลูกค้า (CS) เพื่อเตรียมจัดส่งคืนให้ลูกค้าต่อไป
                </p>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 border-t-4 border-t-[#c8102e] rounded-2xl p-6 md:p-8 shadow-sm">
                <form className="space-y-8">
                    ...
                    {/* Section 3: GR Return details */}
                    <div>
                        <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100 mb-6">
                            <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                            <h3 className="text-sm font-bold text-slate-800">
                                🚚 บันทึกข้อมูลการส่งของคืนให้ฝ่าย CS
                            </h3>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label htmlFor="grSentName" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้ส่งมอบ (เจ้าหน้าที่ GR)<Req /></label>
                                <input
                                    id="grSentName"
                                ...
                                />
                                {errors.grSentName && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.grSentName}</p>}
                            </div>
                            <div>
                                <label htmlFor="receiverName" className="block text-xs font-bold text-slate-700 mb-1.5">ผู้รับสินค้า (เจ้าหน้าที่ CS)<Req /></label>
                                <input
                                    id="receiverName"
                            ...
                            <div>
                                <label htmlFor="grReturnDate" className="block text-xs font-bold text-slate-700 mb-1.5">วันที่รับมอบคืน CS<Req /></label>
                                <DatePicker
                                    id="grReturnDate"
                                ...
                                />
                                {errors.grReturnDate && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.grReturnDate}</p>}
                            </div>
                            <div>
                                <label htmlFor="grSentTel" className="block text-xs font-bold text-slate-700 mb-1.5">เบอร์ติดต่อผู้ส่งคืน (GR)<Req /></label>
                                <input
                                    id="grSentTel"
                                ...
                                />
                                {errors.grSentTel && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.grSentTel}</p>}
                            </div>
                            <div>
                                <label htmlFor="location" className="block text-xs font-bold text-slate-700 mb-1.5">คลังจัดเก็บสินค้า (Location)<Req /></label>
                                <input
                                    id="location"
                                ...
                                />
                                {errors.location && <p className="text-red-600 text-[10px] font-bold mt-1">{errors.location}</p>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between pb-2.5 border-b border-slate-100 mb-6">
                            <div className="flex items-center gap-2">
                                <span className="w-1 h-4 bg-[#c8102e] rounded-full inline-block"></span>
                                <h3 className="text-sm font-bold text-slate-800">
                                    📷 ถ่ายภาพตรวจรับสินค้าของแผนก CS (Attachments)*
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {renderUploadSlot('slot4', 'ใบนำส่งคืน แผนก CS สาขา*', true, fileSlot4, previewSlot4, setFileSlot4, slot4Ref)}
                        </div>
                        {errors.attachments && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider hidden md:inline-block">ขั้นตอนปัจจุบัน: GR Return to CS</span>
                    <button
                        type="button"
                        onClick={onSave}
                    ...
                    >
                        บันทึกของคืนถึง CS สำเร็จ <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                </div>
            </div>
